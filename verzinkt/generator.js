class Generator {
    ...
    async generate() {
        for (let i = 0; i < this.settings.spawn.amount; i++) {
            const position = { x: Generator.randomInt((this.settings.width - this.settings.spawn.width) / 2, (this.settings.width + this.settings.spawn.width) / 2 - 1), y: Generator.randomInt((this.settings.height - this.settings.spawn.height) / 2, (this.settings.height + this.settings.spawn.height) / 2 - 1) };
            const growthValues = {
            ...
            const directionalGrowth = { x: Generator.notZero(Math.cos(growthValues.angle) * growthValues.growthRate), y: Generator.notZero(Math.sin(growthValues.angle) * growthValues.growthRate) }
            const pixelGrowth = {
                up: Math.ceil(Math.abs(directionalGrowth.y > 0 ? directionalGrowth.y : directionalGrowth.y * growthValues.oppositeGrowth)),
                down: Math.ceil(Math.abs(directionalGrowth.y < 0 ? directionalGrowth.y : directionalGrowth.y * growthValues.oppositeGrowth)),
                left: Math.ceil(Math.abs(directionalGrowth.x < 0 ? directionalGrowth.x : directionalGrowth.x * growthValues.oppositeGrowth)),
                right: Math.ceil(Math.abs(directionalGrowth.x > 0 ? directionalGrowth.x : directionalGrowth.x * growthValues.oppositeGrowth))
            }
            const color = `rgb(${Math.floor(55 + (growthValues.angle / 360) * 170)}, ${Math.floor(55 + (growthValues.angle / 360) * 170)}, ${Math.floor(55 + (growthValues.angle / 360) * 170)})`
